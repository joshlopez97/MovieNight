...
public class ResultCodes {
    ...
    public static final int EMAIL_NOT_FOUND_HEADER = -16;
    public static final int SESSIONID_NOT_FOUND_HEADER = -17;
    ...
    public static String setMessage(int code) {
        switch (code) {
            ...
            case SESSIONID_NOT_FOUND_HEADER:
                return "SessionID not provided in request header.";
            case EMAIL_NOT_FOUND_HEADER:
    ...
    public static Response generateResponse(int code)
    {
        Response.Status httpstatus = code > 0 ? Response.Status.OK : Response.Status.BAD_REQUEST;
        return Response.status(httpstatus).entity(new JsonResponse(code, setMessage(code)).toString()).build();
    }
}
