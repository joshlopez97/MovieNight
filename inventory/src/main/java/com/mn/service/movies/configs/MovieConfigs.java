    ...
    public MovieConfigs(ConfigsModel cm) throws NullPointerException {
        if (cm == null) {
            ...
            idmConfigs = new IDMConfigs(
                    cm.getIdmConfig().get("scheme"),
                    cm.getIdmConfig().get("hostName"),
                    Integer.parseInt(cm.getIdmConfig().get("port")),
                    cm.getIdmConfig().get("path"),
                    cm.getIdmEndpoints().get("privilege")
            );
        }
    }
