...
function addField(popup, label, type, currentValue="", name=label)
{
  popup.find(".popup-fields").append(`
    <div class="popup-form-field">
      ...
      <input class="popup-form-field-input" type="${type}" name="${name}" value="${currentValue}">
    </div>
  `);
}
...
function getFormData(popup)
{
  ...
  {
    form_data[raw_data[i]['name']] = raw_data[i]['value'];
  }
  return form_data;
}
  ...
  $("body").append(`
    <div class='notice'>
      ...
      <div id="close-notice-btn" class="close-icon-holder">
        <img class="close-notice" src="img/close-black.png" alt="close">
      </div>
    </div>
  `);
  let n = $(".notice");
